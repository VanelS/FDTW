...
H               <- function(training_set, test_set, code_distance, nom_dataset, n, tab_W, bol_diviseur, nb_iter){
  ...
  return(list( max_local = max, tab_W = tab_W))
}
...
f              <- function(training_set, test_set, code_distance, nom_dataset, bol_diviseur = 0){
  ...
  aux  <- H(training_set, test_set, code_distance, nom_dataset, n, tab_W, bol_diviseur, 1)
  result[[1]]  <- aux$max_local
...
sink(file="Heuristique_puissances_de_deux081016-2.txt")
...
heure1<-Sys.time()
f(DiatomSizeReduction_TRAIN, DiatomSizeReduction_TEST, 2, 'yoga')
heure2<-Sys.time()
t <- difftime(heure2, heure1, units = "mins")
writeLines("\n")
...
heure1<-Sys.time()
f(BirdChicken_TRAIN, BirdChicken_TEST, 2, 'yoga')
heure2<-Sys.time()
t <- difftime(heure2, heure1, units = "mins")
writeLines("\n")
print(paste("Le temps (minutes) d'execution de l'heuristique est : ", t))
